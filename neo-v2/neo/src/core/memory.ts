// src/core/memory.ts
// NEO Memory Engine — save, recall, extract
...
export async function recallMemories(
  ...
  // Generate embedding สำหรับ query
...
// ─────────────────────────────────────────
// EXTRACT — auto-extract memory จาก conversation
// (รันใน background หลัง AI ตอบ)
// ─────────────────────────────────────────
export async function extractMemoriesFromConversation(
  ...
  // Pattern matching ง่ายๆ ก่อน (ไม่ต้องใช้ AI ทุกครั้ง)
  ...
  // Detect "จำไว้ด้วย" / "remember" / "note"
...
// Embedding generator (ใช้ Ollama nomic-embed หรือ OpenAI)
async function generateEmbedding(text: string): Promise<number[]> {
  try {
    // ใช้ Ollama local embedding ก่อน ($0)
    ...
    // Fallback: zero vector (ถ้า embedding ไม่พร้อม)
